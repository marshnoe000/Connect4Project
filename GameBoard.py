...
class GameBoard:
    def __init__(self, rows=6, cols=7):
        ...
        self.tupleWinningSequence = []
        self.totalTurns = 0

    def printBoardForDebuggingOrBasicPlay(self):
    ...
    def getValidMoves(self):
        validMoves = []
        for col in [3, 2, 4, 0, 1, 5, 6]:  # Ordering is based on most valuable columns first to optimize pruning
        ...
        for row in range(self.rows):
            for col in range(self.cols - 3):
                if (self.grid[row][col] == player.token and
                    ...
                    self.tupleWinningSequence = [(row, col), (row, col + 1), (row, col + 2), (row, col + 3)]
                    return True
        return False
        ...
        for row in range(self.rows - 3):
            for col in range(self.cols):
                if (self.grid[row][col] == player.token and
                    ...
                    self.tupleWinningSequence = [(row, col), (row + 1, col), (row + 2, col), (row + 3, col)]
                    return True
        return False
        ...
        for row in range(self.rows - 3):
            for col in range(3, self.cols):
                if (self.grid[row][col] == player.token and
                        ...
                        self.grid[row + 2][col - 2] == player.token and
                        self.grid[row + 3][col - 3] == player.token):
                    self.tupleWinningSequence = [(row, col), (row + 1, col - 1), (row + 2, col - 2), (row + 3, col - 3)]
                    return True
